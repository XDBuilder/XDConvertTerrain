...
bool CZipArchive::Open(LPCTSTR szPathName, int iMode, int iVolumeSize)
{
	...
	bool ret = m_storage.Open(szPathName, iMode, iVolumeSize);
...
bool CZipArchive::ExtractToPath(LPCTSTR szFileName, LPCTSTR szPathName)
{
	Close();	
	if(Open(szFileName, CZipArchive::open, 0 ) == false) 
		return false;//파일이 완전히 받아지지 않았을때....
	for (int j = 0; j < GetNoEntries(); j++)
	{
		CFileHeader fh;
		GetFileInfo(fh, (WORD)j);
		if(ExtractFile((WORD)j, szPathName) == false) 
			return false;
	}
	Close();	
...
//		CFile f(szFile, CFile::modeWrite | CFile::modeCreate | CFile::shareDenyWrite);
		CFile f(szFile,CFile::modeWrite | CFile::modeCreate );
		DWORD iRead;
...
char* CZipArchive::GetFileName(CString str)
{
	int i,cut_count=0;
	for( i=0;i< str.GetLength()-1 ; i++){
		if ( str.GetAt(i) == '\\' ) cut_count=i;
	}
	str=str.Mid(cut_count+1,str.GetLength()-(cut_count+2)+1);
	char *file_name= NULL;
	file_name=new char[str.GetLength()+1];
	for(i=0;i< str.GetLength() ; i++){
		file_name[i]=str.GetAt(i);
	}
	file_name[i]='\0';
	return file_name;
...
bool CZipArchive::AddNewFile(LPCTSTR lpszFilePath, LPCTSTR zipPath, int iLevel, unsigned long nBufSize)
{
	if (!nBufSize)
		return false;
	
	CFileHeader header;
	char* fileName = GetFileName((CString)lpszFilePath);//디렉토리를 제외한 파일 명
	header.m_szFileName.Format("%s\\%s",zipPath,fileName);
