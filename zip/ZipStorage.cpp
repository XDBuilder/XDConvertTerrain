...
bool CZipStorage::Open(LPCTSTR szPathName, int iMode, int iVolumeSize)
{
	...
	if ((iMode == CZipArchive::create) ||(iMode == CZipArchive::createSpan)) // create new archive
	{
		...
		{
			m_iSpanMode = noSpan;
			ret = OpenFile(szPathName, CFile::modeCreate | CFile::modeReadWrite);
			return ret;
		}
	...
	{
		ret = OpenFile(szPathName, CFile::modeNoTruncate | ((iMode == CZipArchive::openReadOnly) ? CFile::modeRead : CFile::modeReadWrite));
