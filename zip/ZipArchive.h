...
//		if iMode == create this argument doesn't matter
	...
	bool ExtractToPath(LPCTSTR szFileName, LPCTSTR szPathName);

	char* GetFileName(CString str);//파일명과 확장자만 가지고 온다..
...
//		the size of the buffer used during compression
	bool AddNewFile(LPCTSTR lpszFilePath, LPCTSTR zipPath, int iLevel = -1, unsigned long nBufSize = 65535);
