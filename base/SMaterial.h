...
#include "matrix4.h"
	...
	enum E_MATERIAL_TYPE
	{
		...
		// This material is currently not implemented in OpenGL, but it works with DirectX.
		EMT_SOLID_2_LAYER,
		...
		// map. For example a value of (127,127,127) will not change the appearance of the diffuse map at all.
	...
	enum E_MATERIAL_FLAG
	{
		...
		// you can use anisotropic texture filtering in
	...
	public:
		...
		SMaterial& operator=(const SMaterial& other)
		{
			...
			for (u32 i=0; i<MATERIAL_MAX_TEXTURES; ++i)
			{
				...
				{
					if (other.TextureMatrix[i])
						TextureMatrix[i] = new matrix4(*other.TextureMatrix[i]);
					else
						TextureMatrix[i] = 0;
				}
		...
		// The default is to reflect white specular light.  See SMaterial::Shininess how to	enable specular lights. 
		...
		//! Second free parameter dependend on the material type. Mostly ignored. 
		...
		//! Texture Matrix array ! Do not acces the elements directly as the internal
		//! ressource management has to cope with Null pointers etc.
		matrix4* TextureMatrix[MATERIAL_MAX_TEXTURES];
		...
		// Default: 1 This flag is ignored, if the MaterialType is a transparent type. 
		bool ZWriteEnable;
		...
		/** you can use anisotropic texture filtering in conjunction with bilinear or trilinear texture
		    filtering to improve rendering results. Primitives will look less blurry with this flag switched on. */
		...
		matrix4& getTextureMatrix(u32 i)
		{
			if (i<MATERIAL_MAX_TEXTURES && !TextureMatrix[i])
				TextureMatrix[i] = new matrix4(matrix4::EM4CONST_IDENTITY);
		...
		void setTextureMatrix(u32 i, const matrix4& mat)
		{
			if (i>=MATERIAL_MAX_TEXTURES)
				return;
			if (!TextureMatrix[i])
				TextureMatrix[i] = new matrix4(mat);
			else
				*TextureMatrix[i] = mat;
		}
		...
		// Equality operator
		inline bool operator==(const SMaterial& b) const { return !(b!=*this); }
